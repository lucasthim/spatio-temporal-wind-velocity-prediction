...
wind_data_mean <- wind_data %>% 
  filter(!is.na(WIND_DIRECTION_degrees) & !NAME %in% c("RIBEIRA DO AMPARO","FEIRA DE SANTANA","EUCLIDES DA CUNHA","QUEIMADAS","CRUZ DAS ALMAS")) %>% 
  group_by(YEAR, MONTH, DAY, NAME, LATITUDE, LONGITUDE) %>% 
  summarise(WIND_DIRECTION_degrees_mean = mean(WIND_DIRECTION_degrees, na.rm = T)) 

wind_data_day <- wind_data_mean %>% 
  ungroup() %>% 
  filter(YEAR > 2017 & YEAR <  2021) %>%
  mutate(DATA = as.POSIXct(paste0(YEAR,"-",MONTH,"-",DAY), format="%Y-%m-%d")) %>% 
  dplyr::select(DATA,  WIND_DIRECTION_degrees_mean, NAME, LATITUDE,LONGITUDE)
...
dados_data <- wind_data_comp %>%
  ...
  pivot_wider(names_from = NAME, values_from = WIND_DIRECTION_degrees_mean) 
...
plot(dados.mat.ts, main = "Direção do Vento")
```
